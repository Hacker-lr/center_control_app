...
class _BigScreenPageState extends State<BigScreenPage> {
  ...
  List<MapEntry<int, String>> _buildLayoutButtonEntries() {
    ...
    if (DeviceConfig.showBigScreenFull) {
      entries.add(const MapEntry(0, '全屏'));
    }
    if (DeviceConfig.showBigScreenFull169) {
      entries.add(const MapEntry(1, '全屏16:9'));
    }
    if (DeviceConfig.showBigScreenSplit2) {
      entries.add(const MapEntry(2, '二分屏'));
    }
    if (DeviceConfig.showBigScreenSplit3) {
      entries.add(const MapEntry(3, '三分屏'));
    }
    if (DeviceConfig.showBigScreenSplit4) {
      entries.add(const MapEntry(4, '四分屏'));
    }
    if (DeviceConfig.showBigScreenSplit5) {
      entries.add(const MapEntry(5, '五分屏'));
    }
    return entries;
  }

  late final List<MapEntry<int, String>> _layoutButtonEntries =
      _buildLayoutButtonEntries();
  ...
  void _onLayoutButtonTapped(int buttonIndex, int layoutKey) {
    ...
    final String command = DeviceConfig.bigScreenSendAsHex
        ...
        ? DeviceConfig.hexBigScreenLayoutCmd.replaceAll(
            '{layout02X}',
            layoutCode.toRadixString(16).padLeft(2, '0').toUpperCase(),
          )
        /// ASCII模式：直接使用十进制数字
        : DeviceConfig.bigScreenLayoutAsciiCmd
