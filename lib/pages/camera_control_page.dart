...
class _CameraControlPageState extends State<CameraControlPage> {
  ...
  int get _currentSpeed =>
      _speedMode == 0 ? DeviceConfig.cameraSpeedLow : DeviceConfig.cameraSpeedHigh;
  ...
  Widget _buildCameraSelection() {
    ...
    return Row(
      ...
      children: List.generate(DeviceConfig.cameraDevices.length, (index) {
        final int camNum = index + 1;
        final bool isSelected = _selectedCamera == camNum;
        return Padding(
          padding: EdgeInsets.symmetric(horizontal: spacing / 2),
          child: _buildSquareBtn(
            label: '$camNum',
            size: btnSize,
            isActive: isSelected,
  ...
  Widget _buildPresetPortrait(double spacing) {
    final double btnSize = ResponsiveUtils.getChannelButtonSize(context) * 0.9;
    return Center(
      child: Wrap(
        alignment: WrapAlignment.center,
        spacing: spacing,
        runSpacing: spacing,
        children: List.generate(DeviceConfig.cameraPresetCount, (index) {
          final int presetNum = index + 1;
          final bool isActive = _activePreset == presetNum;
          return _buildSquareBtn(label: '$presetNum', size: btnSize, isActive: isActive, activeColor: const Color(0xFF3E6B48), onTap: () => _onPresetTapped(presetNum));
        }),
      ),
    );
  }
  ...
  Widget _buildPresetLandscape(double spacing) {
    final double btnSize = ResponsiveUtils.getChannelButtonSize(context) * 0.75;
    return Center(
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: List.generate(DeviceConfig.cameraPresetCount, (index) {
            ...
            return Padding(
              padding: EdgeInsets.symmetric(horizontal: spacing / 2),
              child: _buildSquareBtn(label: '$presetNum', size: btnSize, isActive: isActive, activeColor: const Color(0xFF3E6B48), onTap: () => _onPresetTapped(presetNum)),
            );
          }),
        ),
      ),
  ...
  Widget _buildSquareBtn({required String label, required double size, required bool isActive, required Color activeColor, required VoidCallback onTap}) {
    return GestureDetector(
      onTap: onTap,
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          color: isActive ? activeColor.withAlpha(230) : const Color(0xFF2A2A3E),
          boxShadow: isActive
              ? [BoxShadow(color: activeColor.withAlpha(100), blurRadius: size * 0.18)]
              : [BoxShadow(color: Colors.black.withAlpha(60), blurRadius: size * 0.08, offset: const Offset(0, 3))],
          border: Border.all(color: isActive ? activeColor : const Color(0xFF3A3F48), width: isActive ? 2.0 : 1.0),
        ),
        child: Center(
          child: Text(label, style: TextStyle(fontSize: size * 0.5, fontWeight: FontWeight.w700, color: isActive ? Colors.white : Colors.grey[400])),
        ),
      ),
    );
  }
