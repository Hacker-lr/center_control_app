...
class CameraConnection extends BaseConnection {
  ...
  /// 由设备配置决定，不再硬编码（默认 TCP 以兼容 VISCA over IP 标准）
  final bool _useTcp;
  ...
  Future<bool> sendViscaCommand(String viscaPayload) async {
    ...
    // 将长度转为2位16进制字符串，不足补0
    final header =
        '01 00 00 ${length.toRadixString(16).padLeft(2, '0')} 00 00 00 01';
  ...
  Future<bool> panTiltMove(
    ...
    final cmd =
        '${_hex(_addr)} 01 06 01 ${_hex(panSpeed)} ${_hex(tiltSpeed)} ${_hex(panDir)} ${_hex(tiltDir)} FF';
...
class CameraConnectionManager extends ChangeNotifier {
  /// 单例实例（私有）
  static final CameraConnectionManager _instance =
  ...
  /// 后续若在配置页增删/修改摄像头（IP、端口、协议等），请调用 [rebuild]
  /// 立即按最新配置重建连接列表，无需重启 App。
  CameraConnectionManager._internal() {
    // 遍历所有配置的摄像头设备
    for (int i = 0; i < _config.cameraDevices.length; i++) {
      ...
      _connections.add(
        CameraConnection(
          ...
          useTcp: dev['useTcp'] as bool? ?? true, // 协议类型（TCP/UDP）
        ),
      );
    }
  }
  ...
  /// 根据当前 DeviceConfig 中的摄像头列表重建所有连接实例
  ///
  /// 解决「配置页修改摄像头后需重启 App 才能生效」的痛点：
  /// 配置页保存摄像头配置（IP/端口/协议/地址）后调用本方法，
  /// 立即按最新的 cameraDevices 重建连接列表，无需重启。
  /// 重建前会断开并清理所有旧连接、重置活跃索引。
  void rebuild() {
    // 记录重建前活跃的摄像头编号（0-based），重建后自动重连，
    // 避免“保存配置（如修改预置位个数 / 摄像头 IP / 协议）后，
    // 摄像头页仍停留但连接被清空 → 状态卡在‘未连接’”的问题
    final int prevActive = _activeIndex;

    // 先断开并清理所有旧连接，释放 socket/定时器
    _disconnectAll();
    ...
    for (int i = 0; i < _config.cameraDevices.length; i++) {
      ...
      _connections.add(
        CameraConnection(
          ...
          useTcp: dev['useTcp'] as bool? ?? true,
        ),
      );
    }

    // 通知 UI 刷新（摄像头选择数量/状态可能已变化）
    notifyListeners();

    // 重建前已有活跃摄像头，且重建后该编号仍有效 → 自动重连，
    // 让停留中的摄像头页立即恢复连接（无需手动点摄像头）。
    // 离开页面时 activeIndex 已为 -1（onDisconnect 调用 disconnectAll），
    // 此时不会误在后台重连，符合“按需连接”策略。
    if (prevActive >= 0 && prevActive < _connections.length) {
      connectCamera(prevActive + 1);
    }
  }
