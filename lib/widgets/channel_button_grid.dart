...
class _ChannelButtonGridState extends State<ChannelButtonGrid> {
  ...
  int get _totalPages =>
      (widget.totalCount + DeviceConfig.gridItemsPerPage - 1) ~/
      DeviceConfig.gridItemsPerPage;
    ...
    if (widget.totalCount <= 8) {
      ...
      return LayoutBuilder(
        builder: (context, constraints) {
          ...
          final double btnW =
              (constraints.maxWidth - 3 * DeviceConfig.gridSpacing4Cross) / 4;
          // 计算按钮高度：可用高度减去1个纵向间距，除以行数，再乘以高度系数
          final double btnH =
              ((constraints.maxHeight - 1 * DeviceConfig.gridSpacing4Main) /
                      DeviceConfig.gridRowCount) *
                  DeviceConfig.gridBtnHeightFactor;
          return _buildGrid(
            count: widget.totalCount,
            crossAxisCount: 4,
            mainSpacing: DeviceConfig.gridSpacing4Main,
            crossSpacing: DeviceConfig.gridSpacing4Cross,
            btnWidth: btnW,
            btnHeight: btnH,
          );
        },
          ...
          child: PageView.builder(
            ...
            itemBuilder: (context, pageIndex) {
              ...
              final int startIndex =
                  pageIndex * DeviceConfig.gridItemsPerPage;
              // 计算当前页的结束索引
              final int endIndex = startIndex + DeviceConfig.gridItemsPerPage;
              ...
              return LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final double btnW =
                      (constraints.maxWidth - 7 * DeviceConfig.gridSpacing8Cross) /
                          8;
                  // 计算按钮高度：可用高度减去1个纵向间距，除以行数，再乘以高度系数
                  final double btnH =
                      ((constraints.maxHeight -
                                  1 * DeviceConfig.gridSpacing8Main) /
                              DeviceConfig.gridRowCount) *
                          DeviceConfig.gridBtnHeightFactor;
                  return _buildGrid(
                    count: displayCount,
                    crossAxisCount: 8,
                    mainSpacing: DeviceConfig.gridSpacing8Main,
                    crossSpacing: DeviceConfig.gridSpacing8Cross,
                    btnWidth: btnW,
                    btnHeight: btnH,
  ...
  Widget _buildGrid({
    ...
    return Padding(
      ...
      padding: const EdgeInsets.symmetric(
        horizontal: DeviceConfig.gridHorizontalPadding,
        vertical: DeviceConfig.gridVerticalPadding,
      ),
      // 居中对齐
      child: Center(
